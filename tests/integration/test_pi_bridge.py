...
def test_pi_execute_via_real_cli():
    ...
    response = (
        AgentCtrl.pi().with_provider("anthropic").with_model("sonnet").with_timeout(120).execute(LIVE_TEST_PROMPT)
    )
...
def test_pi_stream_via_real_cli():
    ...
    result = AgentCtrl.pi().with_provider("anthropic").with_model("sonnet").with_timeout(120).stream(LIVE_TEST_PROMPT)
    events = list(result)
